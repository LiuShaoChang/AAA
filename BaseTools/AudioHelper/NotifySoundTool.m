...
static NSInteger lastSoundTaskId = NSIntegerMax;
...
+(void)asyncPlayingWithUrl:(NSString *)url
    ...
    [NotifySoundTool asyncPlayingWithUrl:url downloadCompletion:nil completion:completon];
}

+(void)asyncPlayingWithUrl:(NSString *)url
        downloadCompletion:(void(^)(NSError *error))downloadCompleton
                completion:(void(^)(NSError *error))completon {
    [NotifySoundTool asyncPlayingWithUrl:url taskId:-1 downloadCompletion:downloadCompleton completion:completon];
}

+(void)asyncPlayingWithUrl:(NSString *)url taskId:(NSInteger)taskId
        downloadCompletion:(void(^)(NSError *error))downloadCompleton
                completion:(void(^)(NSError *error))completon {
    NSURL *URL = [NSURL URLWithString:url];
    
    if (URL == nil) {
        NSError *error = [[NSError alloc] initWithDomain:@"yjyx.com" code:NSURLErrorBadURL userInfo:@{NSLocalizedDescriptionKey:@"URL无效"}];
        if (completon) {
            completon(error);
        }
        return;
    ...
    lastSoundTaskId = taskId;
    
    NSArray *parts = [url componentsSeparatedByString:@"/"];
    NSString *filename = [parts lastObject];
    
    NSString *cachesPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject];
    NSString *amrPath = [cachesPath stringByAppendingPathComponent:filename];
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:amrPath]) {
        ...
        NSURLSessionDownloadTask *downloadTask = [sessionManager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            if (downloadCompleton) {
                downloadCompleton(error);
            }
            if (error) {
                completon(error);
                return;
