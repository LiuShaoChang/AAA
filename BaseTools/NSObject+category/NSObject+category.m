...
- (NSString *)filterEmojis{
    __block NSString *filteredString = self;
 ...
 * @returned    整除后的字段
 */
- (NSString *)handleLastZero{

    NSString *returnStr = [NSString stringWithFormat:@"%@", @(self.floatValue)];
    return returnStr;
...
// 四舍五入
+ (NSString *)decimalsRounding:(CGFloat)num
{
    NSString *str = [NSString stringWithFormat:@"%.1f", num];
    NSMutableArray *tmpArr = [NSMutableArray arrayWithArray:[str componentsSeparatedByString:@"."]];
    if ([tmpArr[1] integerValue] >= 5 ) {
        tmpArr[0] = @([tmpArr[0] integerValue] + 1);
    }
    return [NSString stringWithFormat:@"%@", tmpArr[0]];
}
